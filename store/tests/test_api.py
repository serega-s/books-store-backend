...
class BooksApiTestCase(APITestCase):
    ...
    def test_read(self):
        url = reverse('book-detail', args=(self.book_1.id,))
        expected_data = {'id': 14, 'name': 'Test book 1', 'price': '25.00', 'author_name': 'Author 1',
                         'annotated_likes': 1, 'rating': '5.00', 'owner_name': 'test_username',
                         'readers': [OrderedDict([('first_name', ''), ('last_name', '')])]}
        response = self.client.get(url)
        self.assertEqual(status.HTTP_200_OK, response.status_code)
        self.assertEqual(expected_data, response.data, response.data)
